...
PACKAGES="jupyter numpy pandas scipy \
    seaborn pycodestyle pylint pillow matplotlib pytorch torchvision torchaudio"
PY_VERSION="3.10"
...
function when_conda_exist {
    ...
    if conda info --envs | grep -iqF 42AI-$USER; then
        ...
        conda create --name 42AI-$USER python=$PY_VERSION $PACKAGES -y
    fi
}
